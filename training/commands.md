...
# Truffle and TestRPC
    ...
    # Truffle Console. It has references to all the smart contracts deployed

    - truffle migrate (to deploy all the contracts)
    - truffle console
    - > web3.eth.accounts
    - > ObjectOrientation.address
    - > ObjectOrientation.at(ObjectOrientation.address).calculateAgentCommission(5000)
    - > ObjectOrientation.at(ObjectOrientation.address).getAgentInformation()
    - # Copy the transaction has from the result and use it in the below command
    - > web3.eth.getTransaction('0xaf5eff6a0351c74680a9f538bbe1604c641ab933eb37374f8bcea32f8cc26c05')

# Deploy Smart Contract on Private Etherum Network
...
- # Copy Calculator.sol file to edxlab3/Calculator.sol
- # Compile the contract and load the script

- (echo -n "var calcOutput=" ; solc --optimize --combined-json abi,bin,interface Calculator.sol) > calc.js
- geth attach
- > loadScript('calc.js')

- # Get the abi and bin of the Contract

- > var calcContractAbi = calcOutput.contracts['Calculator.sol:Calculator'].abi
- > var calcContract = eth.contract(JSON.parse(calcContractAbi))
- > var calcBinCode = "0x" + calcOutput.contracts['Calculator.sol:Calculator'].bin

- # Deploy the contract

- > personal.unlockAccount(eth.accounts[0], "[password]")
- > var txObject = { from: eth.accounts[0], data: calcBinCode, gas: 1000000 };
- > var calcInstance = calcContract.new(5, txObject)
- > eth.getTransactionReceipt(calcInstance.transactionHash);
- > var calcAddress = eth.getTransactionReceipt(calcInstance.transactionHash).contractAddress
- > var calculator = calcContract.at(calcAddress);
- > calculator.get.call()
- > calculator.set.sendTransaction(100, {from: eth.accounts[0], gas: 1000000})
- > calculator.get.call()
