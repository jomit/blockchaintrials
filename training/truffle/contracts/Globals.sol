...
contract Globals {
  ...
  function  etherUnitsTest() returns(bool) {
   ...
   bool flag = (1 ether == 1000 finney);  // True
   flag = (1 wei == 0.000000000000000001 ether);  // True

   uint amount = 10 ether; 
   flag = (amount*2 == 20000 finney);  // True
  ...
  function  getBlockInformation() returns (uint number, bytes32 hash, address coinbase, uint difficulty) {
    ...
    hash = block.blockhash(number - 1); // -1 because excluding current or else it would return zeros
  ...
  function  callCrypto(string str) returns (bytes32[5] dat){
    ...
    dat[1] = sha3(str);
    dat[2] = sha256(str);
    dat[3] = sha3("AB","C","DE");  // Tightly Packed example:  same as sha3("ABCDE")
    dat[4] = ripemd160(str);
  }
